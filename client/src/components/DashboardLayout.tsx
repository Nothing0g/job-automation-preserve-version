...
function ThemeToggle({ compact = false }: { compact?: boolean }) {
  ...
  return <button onClick={toggleTheme} aria-label={nextLabel} title={nextLabel} className={`group flex items-center gap-2 rounded-md border border-border bg-transparent p-2 text-muted-foreground transition-colors duration-200 hover:bg-sidebar-accent hover:text-foreground ${compact ? "h-9 w-9 justify-center" : "w-full"}`}><span className="relative flex h-5 w-5 items-center justify-center"><Sun className={`absolute h-4 w-4 transition-all ${theme === "light" ? "scale-100 opacity-100" : "scale-75 opacity-0"}`} /><Moon className={`absolute h-4 w-4 transition-all ${theme === "dark" ? "scale-100 opacity-100" : "scale-75 opacity-0"}`} /></span>{!compact && <span className="text-sm font-medium">{theme === "light" ? "Light mode" : "Dark mode"}</span>}</button>;
}
