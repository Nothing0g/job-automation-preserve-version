...
import { ArrowRight, CalendarClock, CheckCircle2, Plus, Sparkles } from "lucide-react";
...
type ApplicationForm = { company: string; role: string; jobDescription: string; status: Status; nextAction: string; followUpAt: string };
const blankForm: ApplicationForm = { company: "", role: "", jobDescription: "", status: "to-apply", nextAction: "", followUpAt: "" };
...
function StartApplicationDialog({ onDone }: { onDone: () => void }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const job = await create.mutateAsync({ ...form, nextAction: form.nextAction || null, followUpAt: form.followUpAt || null });
      if (!job) throw new Error("The application could not be created.");
      onDone();
      setOpen(false);
      setForm(blankForm);
      toast.success("Application created. Preparing your tailored drafts…");
      try {
        await generate.mutateAsync({ id: job.id });
        toast.success("Your tailored resume and outreach email are ready to review.");
      } catch (error) {
        toast.error(error instanceof Error ? error.message : "The job was saved, but its drafts need another try.");
      }
      setLocation(`/jobs/${job.id}`);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "The application could not be created.");
    }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="editorial-title text-3xl">Start an application</DialogTitle>
          <DialogDescription>Enter the essentials once. The studio saves the job, then creates your factual tailored resume and personalized outreach draft from your master profile.</DialogDescription>
        </DialogHeader>
        <form onSubmit={submit} className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2"><Label htmlFor="company">Company</Label><Input id="company" value={form.company} onChange={event => setForm({ ...form, company: event.target.value })} required /></div>
            <div className="space-y-2"><Label htmlFor="role">Role title</Label><Input id="role" value={form.role} onChange={event => setForm({ ...form, role: event.target.value })} required /></div>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2"><Label htmlFor="status">Starting stage</Label><select id="status" className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={form.status} onChange={event => setForm({ ...form, status: event.target.value as Status })}>{Object.entries(statusConfig).map(([value, config]) => <option key={value} value={value}>{config.label}</option>)}</select></div>
            <div className="space-y-2"><Label htmlFor="follow-up">Follow-up date <span className="text-muted-foreground">(optional)</span></Label><Input id="follow-up" type="date" value={form.followUpAt} onChange={event => setForm({ ...form, followUpAt: event.target.value })} /></div>
          </div>
          ...
          <div className="space-y-2"><Label htmlFor="description">Job description</Label><Textarea id="description" value={form.jobDescription} onChange={event => setForm({ ...form, jobDescription: event.target.value })} className="min-h-56 leading-6" placeholder="Paste the full job description here…" required /></div>
          <DialogFooter><Button type="submit" disabled={busy}><Sparkles className="mr-2 h-4 w-4" />{create.isPending ? "Saving application…" : generate.isPending ? "Creating drafts…" : "Save and create drafts"}</Button></DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
  );
...
export default function JobsDashboard() {
  ...
  return (
    <div className="mx-auto max-w-[1600px] space-y-8">
      <section className="relative overflow-hidden rounded-[1.8rem] border border-border bg-[radial-gradient(circle_at_86%_10%,oklch(0.88_0.07_70),transparent_27%),linear-gradient(135deg,oklch(0.99_0.006_80),oklch(0.955_0.018_72))] px-6 py-8 sm:px-9 sm:py-10 dark:bg-[radial-gradient(circle_at_86%_10%,oklch(0.36_0.07_55),transparent_30%),linear-gradient(135deg,oklch(0.18_0.022_40),oklch(0.22_0.026_42))]">
        <div className="absolute -right-8 -top-10 h-44 w-44 rounded-full border border-primary/20" />
        <p className="data-label text-primary">Personal application studio</p>
        <div className="mt-3 flex flex-col justify-between gap-6 lg:flex-row lg:items-end"><div><h1 className="editorial-title max-w-2xl text-5xl leading-[0.94] sm:text-6xl">Make each application feel considered.</h1><p className="mt-4 max-w-xl text-sm leading-6 text-muted-foreground">Add the role details, let the studio shape the first drafts, and keep every next move in clear view.</p></div><StartApplicationDialog onDone={refresh} /></div>
      </section>
      ...
      <section>
        <div className="mb-4 flex items-end justify-between"><div><p className="data-label text-primary">Pipeline</p><h2 className="editorial-title mt-1 text-4xl">Your active applications</h2></div>{jobs.length > 0 && <Badge variant="secondary" className="rounded-full px-3 py-1">{jobs.length} total</Badge>}</div>
        {isLoading ? <div className="grid gap-4 xl:grid-cols-5">{Object.keys(statusConfig).map(status => <div key={status} className="h-56 animate-pulse rounded-2xl border bg-card/60" />)}</div> : jobs.length === 0 ? <div className="paper-panel flex min-h-80 flex-col items-center justify-center rounded-[1.5rem] border border-dashed px-6 text-center"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-accent text-accent-foreground"><CheckCircle2 className="h-5 w-5" /></div><h3 className="editorial-title mt-5 text-3xl">Ready when you are.</h3><p className="mt-2 max-w-sm text-sm leading-6 text-muted-foreground">Start with one role. Your tailored materials and follow-up plan will live in one place.</p></div> : <div className="grid gap-4 xl:grid-cols-5">{grouped.map(column => <div key={column.status} className="rounded-2xl border border-border/80 bg-card/45 p-3"><div className="mb-3 flex items-center gap-2 px-1"><span className={`h-2 w-2 rounded-full ${statusConfig[column.status].accent}`} /><span className="text-sm font-semibold">{statusConfig[column.status].label}</span><span className="ml-auto text-xs text-muted-foreground">{column.jobs.length}</span></div><div className="space-y-3">{column.jobs.map(job => <Link key={job.id} href={`/jobs/${job.id}`} className="paper-panel group block rounded-xl border p-4 transition duration-200 hover:-translate-y-0.5 hover:border-primary/35"><p className="data-label text-muted-foreground">{job.company}</p><h3 className="mt-2 line-clamp-2 text-base font-semibold leading-5 tracking-tight">{job.role}</h3>{job.nextAction && <p className="mt-3 line-clamp-2 text-xs font-medium leading-5 text-primary">Next: {job.nextAction}</p>}<p className="mt-3 line-clamp-2 text-xs leading-5 text-muted-foreground">{job.jobDescription}</p><div className="mt-4 flex items-center text-xs font-medium text-primary opacity-0 transition-opacity group-hover:opacity-100">Open workspace <ArrowRight className="ml-1 h-3.5 w-3.5" /></div></Link>)}</div></div>)}</div>}
      </section>
    </div>
  );
}
