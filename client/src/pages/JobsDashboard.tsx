...
import { groupFollowUpReminders } from "@/lib/reminders";
import { applyManualPostingContext, hasSufficientManualContext } from "@/lib/intakeFallback";
import { downloadTrackerWorkbook } from "@/lib/trackerExport";
import { AlertTriangle, ArrowRight, CalendarClock, CheckCircle2, CircleDashed, Download, FileSearch, FileSpreadsheet, Link2, Mail, Plus, ShieldCheck, Sparkles } from "lucide-react";
...
type ApplicationForm = { company: string; role: string; jobDescription: string; manualContext: string; contextMode: "full" | "limited"; contactEmail: string; sourceUrl: string; status: Status; nextAction: string; followUpAt: string };
const blankForm: ApplicationForm = { company: "", role: "", jobDescription: "", manualContext: "", contextMode: "full", contactEmail: "", sourceUrl: "", status: "to-apply", nextAction: "", followUpAt: "" };
...
function StartApplicationDialog({ onDone }: { onDone: () => void }) {
  ...
  const [linkImportError, setLinkImportError] = useState(false);
  ...
  async function importFromLink() {
    ...
    try {
      ...
      setLinkImportError(true);
      toast.error(error instanceof Error ? error.message : "This public job page could not be imported.");
    }
  }

  function usePastedContext() {
    if (!hasSufficientManualContext(form.manualContext)) return toast.error("Paste at least 40 characters from the job page so the draft has useful context.");
    setForm(current => applyManualPostingContext(current, current.manualContext));
    setMode("details");
    toast.success("Pasted context is ready to review as the job description.");
  }

  async function submit(event: FormEvent) {
    event.preventDefault();
    if (!form.contactEmail.trim()) return toast.error("Add the company or hiring email so Gmail can prefill the recipient.");
    try {
  ...
  return <Dialog open={open} onOpenChange={setOpen}>
    ...
    <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
      ...
      <form onSubmit={submit} className="space-y-4">
        {mode === "link" && <section className="rounded-xl border border-primary/20 bg-accent/35 p-4"><div className="flex gap-3"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-primary" /><div><p className="text-sm font-semibold">Import from a public page</p><p className="mt-1 text-xs leading-5 text-muted-foreground">The studio reads exposed job-page text only. Pages that require a login or load details in the browser may need manual entry.</p></div></div><div className="mt-4 flex gap-2"><Input aria-label="Public job posting URL" type="url" placeholder="https://careers.example.com/jobs/..." value={form.sourceUrl} onChange={event => setForm({ ...form, sourceUrl: event.target.value })} /><Button type="button" variant="outline" className="shrink-0 bg-card" disabled={importPosting.isPending} onClick={() => void importFromLink()}>{importPosting.isPending ? "Reading…" : "Import"}</Button></div>{linkImportError && <div className="mt-4 rounded-lg border border-amber-500/25 bg-amber-50/70 p-3 dark:bg-amber-950/20"><p className="text-sm font-semibold text-amber-900 dark:text-amber-100">Paste the visible job text instead</p><p className="mt-1 text-xs leading-5 text-amber-800/80 dark:text-amber-100/70">Copy the role requirements or full posting text from the page. Company and role remain editable below.</p><Textarea value={form.manualContext} onChange={event => setForm({ ...form, manualContext: event.target.value })} className="mt-3 min-h-36 bg-background/80 leading-5" placeholder="Paste any visible role details, requirements, or responsibilities…" /><Button type="button" size="sm" variant="outline" className="mt-3 bg-card" onClick={usePastedContext}>Use pasted text</Button></div>}</section>}
        ...
        <div className="space-y-2"><Label htmlFor="contact-email">Company or hiring email <span className="text-muted-foreground">(required)</span></Label><Input id="contact-email" type="email" placeholder="hiring@company.com" value={form.contactEmail} onChange={event => setForm({ ...form, contactEmail: event.target.value })} required /><p className="text-xs text-muted-foreground">This pre-fills the recipient in your manual Gmail draft. You can edit it later in the application workspace.</p></div>
...
export default function JobsDashboard() {
  ...
  const reminderGroups = useMemo(() => groupFollowUpReminders(jobs), [jobs]);
  ...
  {(reminderGroups.overdue.length + reminderGroups.today.length + reminderGroups.upcoming.length + reminderGroups.unscheduled.length > 0) && <section className="paper-panel rounded-[1.4rem] border p-5 sm:p-6"><div className="flex items-end justify-between"><div><p className="data-label text-primary">Follow-up reminders</p><h2 className="editorial-title mt-1 text-3xl">Know exactly what comes next</h2></div><Badge variant="secondary" className="rounded-full px-3 py-1">{reminderGroups.overdue.length + reminderGroups.today.length + reminderGroups.upcoming.length + reminderGroups.unscheduled.length} active</Badge></div><div className="mt-5 grid gap-4 xl:grid-cols-4">{([{ key: "overdue", label: "Overdue", icon: AlertTriangle, tone: "border-rose-500/25 bg-rose-50/60 dark:bg-rose-950/15", iconTone: "bg-rose-100 text-rose-700 dark:bg-rose-950 dark:text-rose-200" }, { key: "today", label: "Due today", icon: CalendarClock, tone: "border-amber-500/25 bg-amber-50/60 dark:bg-amber-950/15", iconTone: "bg-amber-100 text-amber-700 dark:bg-amber-950 dark:text-amber-200" }, { key: "upcoming", label: "Upcoming", icon: CalendarClock, tone: "border-sky-500/20 bg-sky-50/60 dark:bg-sky-950/15", iconTone: "bg-sky-100 text-sky-700 dark:bg-sky-950 dark:text-sky-200" }, { key: "unscheduled", label: "Needs a date", icon: CircleDashed, tone: "border-border bg-card/55", iconTone: "bg-accent text-accent-foreground" }] as const).map(group => <div key={group.key} className={`rounded-xl border p-3 ${group.tone}`}><div className="flex items-center gap-2"><span className={`flex h-7 w-7 items-center justify-center rounded-lg ${group.iconTone}`}><group.icon className="h-3.5 w-3.5" /></span><p className="text-sm font-semibold">{group.label}</p><span className="ml-auto text-xs text-muted-foreground">{reminderGroups[group.key].length}</span></div><div className="mt-3 space-y-2">{reminderGroups[group.key].slice(0, 3).map(job => <Link key={job.id} href={`/jobs/${job.id}`} className="block rounded-lg border bg-card/75 px-3 py-2 transition-colors hover:border-primary/35"><p className="data-label text-muted-foreground">{job.company}{job.followUpAt ? ` · ${displayDate(job.followUpAt)}` : ""}</p><p className="mt-1 line-clamp-2 text-xs font-semibold leading-5">{job.nextAction || `Review ${job.role}`}</p></Link>)}{reminderGroups[group.key].length === 0 && <p className="rounded-lg border border-dashed px-3 py-3 text-xs leading-5 text-muted-foreground">Nothing here right now.</p>}{reminderGroups[group.key].length > 3 && <p className="px-1 text-xs text-muted-foreground">+{reminderGroups[group.key].length - 3} more</p>}</div></div>)}</div></section>} 
  <section><div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="data-label text-primary">Pipeline</p><h2 className="editorial-title mt-1 text-4xl">Your active applications</h2></div><div className="flex flex-wrap items-center gap-2">{jobs.length > 0 && <Badge variant="secondary" className="rounded-full px-3 py-1">{jobs.length} total</Badge>}<Button type="button" size="sm" variant="outline" className="bg-card" disabled={!jobs.length} onClick={() => { downloadTrackerWorkbook(jobs); toast.success("Your tracker spreadsheet is downloading."); }}><FileSpreadsheet className="mr-1.5 h-3.5 w-3.5" />Download Excel</Button></div></div>{isLoading ? <div className="grid gap-4 xl:grid-cols-5">{Object.keys(statusConfig).map(status => <div key={status} className="h-56 animate-pulse rounded-2xl border bg-card/60" />)}</div> : jobs.length === 0 ? <div className="paper-panel flex min-h-80 flex-col items-center justify-center rounded-[1.5rem] border border-dashed px-6 text-center"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-accent text-accent-foreground"><CheckCircle2 className="h-5 w-5" /></div><h3 className="editorial-title mt-5 text-3xl">Ready when you are.</h3><p className="mt-2 max-w-sm text-sm leading-6 text-muted-foreground">Start with a full posting, a public link, or just a role and contact email.</p></div> : <div className="grid gap-4 xl:grid-cols-5">{grouped.map(column => <div key={column.status} className="rounded-2xl border border-border/80 bg-card/45 p-3"><div className="mb-3 flex items-center gap-2 px-1"><span className={`h-2 w-2 rounded-full ${statusConfig[column.status].accent}`} /><span className="text-sm font-semibold">{statusConfig[column.status].label}</span><span className="ml-auto text-xs text-muted-foreground">{column.jobs.length}</span></div><div className="space-y-3">{column.jobs.map(job => <Link key={job.id} href={`/jobs/${job.id}`} className="paper-panel group block rounded-xl border p-4 transition duration-200 hover:-translate-y-0.5 hover:border-primary/35"><p className="data-label text-muted-foreground">{job.company}</p><h3 className="mt-2 line-clamp-2 text-base font-semibold leading-5 tracking-tight">{job.role}</h3>{job.contextMode === "limited" ? <p className="mt-3 text-xs font-medium leading-5 text-amber-700 dark:text-amber-300">Limited context · email-first</p> : <p className="mt-3 line-clamp-2 text-xs leading-5 text-muted-foreground">{job.jobDescription}</p>}{job.nextAction && <p className="mt-3 line-clamp-2 text-xs font-medium leading-5 text-primary">Next: {job.nextAction}</p>}<div className="mt-4 flex items-center text-xs font-medium text-primary opacity-0 transition-opacity group-hover:opacity-100">Open workspace <ArrowRight className="ml-1 h-3.5 w-3.5" /></div></Link>)}</div></div>)}</div>}</section></div>;
}
